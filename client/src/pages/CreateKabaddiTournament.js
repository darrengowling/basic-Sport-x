...
import KabaddiPlayerSelection from '../components/KabaddiPlayerSelection';
import { 
  Target, 
  Calendar, 
  Users, 
...
const CreateKabaddiTournament = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    entryFee: 5,
    maxParticipants: 8,
    budget: 30000000, // £30m default for Kabaddi
    squadRules: {
      raiders: 4,
      defenders: 4,
      allRounders: 4,
      totalPlayers: 12 // 7 playing + 5 subs
    },
    auctionSettings: {
      bidIncrement: 25000, // £25k
      bidTimeout: 30, // 30 seconds
      minimumBid: 50000 // £50k
    },
    auctionDate: '',
    tournamentStart: '',
    tournamentEnd: '',
  ...
  const fetchRealTournaments = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_SERVER_URL || 'http://localhost:8001'}/api/real-kabaddi-tournaments`);
      setRealTournaments(response.data);
    } catch (error) {
      console.error('Error fetching real kabaddi tournaments:', error);
    }
  };
  ...
  const validateStep = (step) => {
    switch (step) {
      case 1:
        ...
        break;
      case 2:
        if (formData.squadRules.raiders + formData.squadRules.defenders + 
            formData.squadRules.allRounders !== formData.squadRules.totalPlayers) {
          toast.error('Team composition must add up to total players');
          return false;
        }
        break;
      case 3:
        if (formData.selectedPlayers.length === 0) {
          toast.error('Please select at least one Kabaddi player for the auction');
          return false;
        }
        break;
      default:
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_SERVER_URL || 'http://localhost:8001'}/api/kabaddi-tournaments`,
        {
          adminId,
          settings: {
            ...formData,
      ...
      if (response.data.success) {
        toast.success('Kabaddi tournament created successfully!');
        navigate(`/kabaddi-tournaments/${response.data.tournament.id}`);
      }
    } catch (error) {
      console.error('Error creating kabaddi tournament:', error);
      toast.error(error.response?.data?.error || 'Failed to create kabaddi tournament');
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 dark:text-gray-300">
              Set up a new Kabaddi tournament for Pro Kabaddi League 2024
            </p>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8">
            <form onSubmit={handleSubmit}>
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="e.g., Friends PKL 2024 Tournament"
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div>
                        ...
                        <input
                          type="number"
                          name="squadRules.raiders"
                          value={formData.squadRules.raiders}
                          onChange={handleInputChange}
                          min="2"
                          className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent text-gray-900 dark:text-white"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          type="number"
                          name="squadRules.defenders"
                          value={formData.squadRules.defenders}
                          onChange={handleInputChange}
                          min="2"
                          className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent text-gray-900 dark:text-white"
                        />
                    ...
                    <div className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                      Total: {formData.squadRules.raiders + formData.squadRules.defenders + 
                              formData.squadRules.allRounders} / {formData.squadRules.totalPlayers} players
                    </div>
                  </div>
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                      <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                          Minimum amount to increase bid
                        </p>
                      </div>
              ...
              {currentStep === 4 && (
                ...
                >
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                    Review Kabaddi Tournament Details
                  </h3>
                    ...
                    <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium text-orange-600 dark:text-orange-400">£{calculatePrizePool()}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium text-orange-600 dark:text-orange-400">{formData.selectedPlayers.length}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-orange-50 dark:bg-orange-900/20 rounded-lg p-6">
                    <h4 className="font-semibold text-orange-900 dark:text-orange-200 mb-4">
                      Selected Kabaddi Players ({formData.selectedPlayers.length})
                    </h4>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                      {['Raider', 'Defender', 'All-rounder'].map(role => {
                        const count = formData.selectedPlayers.filter(p => p.role === role).length;
                        return (
                          <div key={role} className="text-center">
                            <div className="text-orange-700 dark:text-orange-300 font-bold text-lg">{count}</div>
              ...
              <div className="flex justify-between pt-8">
                ...
                {currentStep < totalSteps ? (
                  <button
                    type="button"
                    onClick={nextStep}
                    className="flex items-center space-x-2 px-6 py-3 bg-orange-600 hover:bg-orange-700 text-white font-semibold rounded-lg transition-colors"
                  >
                    <span>Next</span>
...
export default CreateKabaddiTournament;